...
async function fetchDiaByDate(dateStr) {
  ...
  if (error && error.code==="PGRST116") {
    const { data:novo, error:e2 } = await supabase.from("dias").insert({date:dateStr, finalizado:false, turno_ativo:1}).select().single();
    if (e2) throw e2;
    dia = novo;
        ...
        <div onClick={()=>setShowFT(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,padding:"2rem",width:400,boxShadow:"0 20px 60px rgba(0,0,0,.18)"}}>
            ...
            <div style={{background:"#f7f5f2",borderRadius:8,padding:"10px 12px",marginBottom:"1.25rem",border:`2px solid ${t.dot}`}}>
              ...
              <div style={{fontSize:13,color:anotAtiva.texto?"#444":"#bbb",lineHeight:1.6,fontStyle:anotAtiva.texto?"normal":"italic"}}>
...
export default function App() {
  ...
  // loading separado: initialLoad (mostra splash) vs dataLoading (operação pontual)
  const [initialLoad, setInitialLoad] = useState(true);
  const [toast, setToast]         = useState(null);
  const dataLoadedRef             = useRef(false); // evita carregar dados duas vezes

  function showToast(msg, type="success") { setToast({msg,type}); setTimeout(()=>setToast(null),3500); }

  // ── Auth: escuta apenas mudanças reais de usuário, ignora TOKEN_REFRESHED ──
  useEffect(()=>{
    // Obtém sessão inicial UMA vez
    supabase.auth.getSession().then(({data:{session:s}})=>{
      setSession(s??null);
    });

    const {data:{subscription}} = supabase.auth.onAuthStateChange((event, s)=>{
      // Ignora eventos que não trocam o usuário — evita re-render ao trocar aba
  ...
  // ── Carrega dados apenas UMA vez quando o userId fica disponível ──
  const userId = session?.user?.id;
  useEffect(()=>{
    if (!userId) return;
    if (dataLoadedRef.current) return; // já carregou, não recarrega ao trocar aba
  ...
  async function handleChangeDia(dateStr) {
    // Não usa setLoading global — evita desmontar a UI inteira
    try {
  ...
  // Splash apenas na carga inicial — nunca desmonta a UI após estar pronta
