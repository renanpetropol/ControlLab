...
// ─── AnotacoesTurnos ──────────────────────────────────────────────────────────
function AnotacoesTurnos({ anotacoes, turnoAtivo, readonly, onChange, onReabrir }) {
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:"0.75rem"}}>
              ...
              {anot.finalizado&&(
                <div style={{display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}>
                  <span style={{fontSize:11,color:"#1a6b3a",background:"#d4f5e0",padding:"2px 8px",borderRadius:8,fontWeight:700}}>✓ Finalizado</span>
                  {!readonly&&onReabrir&&(
                    <button
                      onClick={()=>onReabrir(t.id)}
                      title="Reabrir anotações deste turno para edição"
                      style={{fontSize:11,color:"#7b4fa6",background:"#ede9fb",border:"none",padding:"2px 10px",borderRadius:8,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}
                    >↩ Reabrir</button>
                  )}
                </div>
              )}
...
// ─── DashboardPage ────────────────────────────────────────────────────────────
function DashboardPage({ dia, onFinalizarTurno, onFinalizarDia, onAddMaterial, onUpdateCell, onEditMaterial, onRemoveMaterial, onLimparDashboard, onAnotacaoChange, onChangeDia, onReobrirTurno }) {
...
export default function App() {
  ...
  async function handleFinalizarTurno() {
    ...
    try {
      ...
      // Avança para o próximo turno (ciclo 1 → 2 → 3 → 1)
      const proximo = turnoNum === 3 ? 1 : turnoNum + 1;
      await supabase.from("dias").update({turno_ativo:proximo}).eq("id",dia.id);
      setDia(prev=>({
        ...prev,
        turnoAtivo: proximo,
  ...
  async function handleReobrirTurno(turnoNum) {
    try {
      await supabase.from("turno_anotacoes").upsert(
        {dia_id:dia.id, turno_num:turnoNum, texto:dia.anotacoes.find(a=>a.turno_num===turnoNum)?.texto||"", finalizado:false},
        {onConflict:"dia_id,turno_num"}
      );
      // Volta o turno ativo para o turno reaberto
      await supabase.from("dias").update({turno_ativo:turnoNum}).eq("id",dia.id);
      setDia(prev=>({
        ...prev,
        turnoAtivo: turnoNum,
        anotacoes: prev.anotacoes.map(a=>a.turno_num===turnoNum?{...a,finalizado:false}:a)
      }));
      showToast(`${TURNOS.find(t=>t.id===turnoNum)?.label} reaberto para edição.`);
    } catch(e) { showToast("Erro ao reabrir turno.","error"); console.error(e); }
  }
