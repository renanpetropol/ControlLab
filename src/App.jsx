...
function AddMaterialModal({ onClose, onAdd }) {
  ...
  async function add() {
    ...
    await onAdd({codigo:codigo.trim(), resina:resina.trim(), cells}); onClose();
  }
  return (
...
export default function App() {
  ...
  // Salva anotação em tempo real (debounce local, salva no blur/finalizar)
  ...
  async function saveAnotacao(turnoNum, texto) {
    try {
      await supabase.from("turno_anotacoes").upsert({dia_id:dia.id, turno_num:turnoNum, texto, finalizado:false},{onConflict:"dia_id,turno_num"});
    } catch(e) { /* silencioso */ }
  }
  ...
  async function handleFinalizarDia() {
    ...
    // Salva todas as anotações ainda não salvas
    for (const a of dia.anotacoes) {
      await saveAnotacao(a.turno_num, a.texto);
    }
    try {
